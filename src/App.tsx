...
// Load Tailwind CSS CDN - crucial for styling
// Note: In a production React app with a build process (like Create React App or Next.js),
// Tailwind would typically be set up differently (e.g., via npm and postcss).
// For a self-contained Canvas example, direct CDN inclusion is used.
// <script src="https://cdn.tailwindcss.com"></script> is expected to be in the HTML head.

// --- Asset URLs ---
const logoUrl = "/WW Forward.png"; // Placeholder for actual logo path

// --- SVG Icons (as functional components) ---
// These icons are defined as React functional components for reusability and clarity.
// They use Tailwind CSS classes for sizing and fill color.
...
// --- Type Definitions ---
// Defining interfaces for component props and data structures for better type safety and readability.
...
/**
 * Renders a decorative dot pattern with optional animation.
 * Used for background visual effects on cards and headers.
 */
...
/**
 * A versatile card component with consistent styling, shadow effects, and optional dot patterns.
 * It provides a standardized look for content blocks across the website.
 */
...
/**
 * A reusable button component with different styles (primary, secondary, success, warning),
 * sizes, and an optional icon.
 */
...
// Props for the GoogleFormComponent, defining the form URL and field mappings.
...
// Props for form field components, passed by GoogleFormComponent.
...
/**
 * A wrapper component for integrating with Google Forms.
 * It handles form data state, submission, and status messages.
 * It uses the `mode: 'no-cors'` for the fetch request as Google Forms typically don't support CORS for direct JS submissions.
 */
const GoogleFormComponent: FC<GoogleFormComponentProps> = ({ formUrl, fieldMapping, children, onSuccess, onError }) => {
    ...
    // Handles changes to form inputs, including checkboxes for multiple selections.
    ...
    // Handles form submission, converting React state to Google Form's expected FormData.
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        e.preventDefault();
        
        // Client-side validation for positionsOfInterest checkbox.
        ...
        for (const key in formData) {
            if (fieldMapping[key]) {
                ...
                if (Array.isArray(value)) {
                    // Append multiple values for checkbox groups.
                    value.forEach(item => {
        ...
        // Set a default position if not selected, as per the original form's logic.
        ...
        try {
            // Using 'no-cors' mode as direct submission to Google Forms often requires it.
    ...
    // Clones children to inject handleChange and formData props.
...
/**
 * Form for suggesting questions to candidates.
 * Uses the GoogleFormComponent for submission.
 */
...
/**
 * Form for volunteers to sign up.
 * Uses the GoogleFormComponent for submission.
 */
...
// --- Data ---
// Centralized data for the forum and projects to easily manage content.
const forumData = {
...
/**
 * Navigation bar component with responsive design and dynamic active page highlighting.
...
const Navbar: FC<NavbarProps> = ({ setActivePage, activePage, selectedProject }) => {
    ...
    // Handles navigation clicks for desktop.
    const handleNav = (item: PageName) => setActivePage(item);
    // Handles navigation clicks for mobile, also closes the menu.
    const handleMobileNav = (item: PageName) => { handleNav(item); setIsMobileMenuOpen(false); };
    // Handles logo click, navigates to Home and closes mobile menu.
...
/**
 * Footer component with copyright, mission statement, and social media links.
 */
const Footer: FC = () => (
...
/**
 * Header component specifically for the Events page (Candidate Forum).
 * Includes event details and an "Add to Calendar" button.
 */
...
/**
 * Section displaying the progress milestones for the Candidate Forum.
...
/**
 * Section detailing the format and structure of the Candidate Forum.
 * Includes a section for forum requirements.
 */
const ForumFormatSection: FC = () => {
    // Helper function to dynamically select icons based on type string.
    const getIcon = (iconType: string) => {
        ...
        return <IconLightBulb />; // Default icon
    };
    return (
...
/**
 * Section showcasing the distinguished panelists for the forum.
 * Each panelist has an expandable bio.
 */
                        ...
                        <div key={panelist.id} className="flex flex-col">
                            <div className={`text-center p-6 rounded-xl border transition-all duration-300 cursor-pointer flex flex-col h-full ${selectedPanelistId === panelist.id ? "border-sky-500 bg-sky-50 shadow-lg" : "border-gray-200 hover:border-sky-300 hover:shadow-md"}`} onClick={() => setSelectedPanelistId(selectedPanelistId === panelist.id ? null : panelist.id)}>
                                ...
                                <div className="flex-grow flex flex-col">
                                    ...
                                    {!panelist.note && <div className="min-h-[2.5rem] mb-3"></div>} {/* Placeholder to maintain layout consistency */}
                                </div>
...
/**
 * Section for user interaction, allowing them to suggest questions or volunteer.
 * Uses tabs to switch between the two forms/options.
 */
const InteractiveSection: FC = () => {
    const [activeTab, setActiveTab] = useState("questions");

    // Configuration for the Google Form for question suggestions.
                ...
                {activeTab === "questions" && (
                    <div>
                        ...
                        <GoogleFormComponent formUrl={questionFormConfig.url} fieldMapping={questionFormConfig.fields}>
                            {/* CandidateQuestionForm expects handleChange and formData props, which are injected by GoogleFormComponent */}
...
/**
 * Section providing important information about the forum, such as attendance,
 * candidate participation, and ground rules. Uses an accordion-like UI.
 */
                    ...
                    {infoSections.map((section) => (
                        <div key={section.id} className="border border-gray-200 rounded-lg overflow-hidden">
                            <button onClick={() => setExpandedSection(expandedSection === section.id ? null : section.id)} className="w-full flex items-center justify-between p-4 text-left hover:bg-gray-50 transition-colors">
                                <div className="flex items-center"> <div className="text-sky-600 mr-2">{section.icon}</div> <span className="font-semibold text-slate-800">{section.title}</span> </div>
...
/**
 * Home page component, featuring a hero section, mission statement,
 * key initiatives, and a call to action.
 */
...
/**
 * About page component, detailing the organization's mission, founders,
...
/**
 * Project card component for displaying a summary of a project.
 * Clicking the card or button navigates to the project's detail page or a related event page.
 */
const ProjectCard: FC<ProjectCardProps> = ({ project, setActivePage }) => {
    // Handles click on the card itself, preventing navigation if a button/link inside is clicked.
    const handleCardClick = (e: React.MouseEvent<HTMLDivElement>) => {
        if ((e.target as HTMLElement).closest('a, button')) return; // Do nothing if a child link/button was clicked
        if (project.redirectTo) setActivePage(project.redirectTo);
        else setActivePage("ProjectDetails", project);
    };
    // Handles click on the "View Details" button, stopping event propagation.
...
/**
 * Projects page component, displaying a grid of ProjectCard components.
 */
...
/**
 * Project detail page component, showing comprehensive information about a specific project.
 * Includes overview, initiatives, timeline, impact, and partners.
 */
                    ...
                    <div className="prose max-w-none mt-6 sm:mt-8 text-gray-700 leading-relaxed text-xs sm:text-sm md:text-base">
                        ...
                        <div className="mt-8 sm:mt-10 p-4 sm:p-6 bg-sky-50 rounded-lg border border-sky-200">
                            ...
                            <div className="flex flex-col sm:flex-row gap-3">
                                ...
                                {/* Only show "Read Full Proposal" button for the Princeton Junction project */}
...
/**
 * Events page component, primarily dedicated to the Candidate Forum.
 * Includes various sections about the forum details, panelists, and ways to get involved.
 */
...
/**
 * Contact page component, featuring contact information and a message form.
 * The form uses Web3Forms for submission.
 */
const ContactPage: FC = () => {
    const initialFormData = { name: "", email: "", message: "" };
    const [formData, setFormData] = useState(initialFormData);
    const [result, setResult] = useState("");
    // Web3Forms access key for form submission.
    const web3FormsAccessKey = "ccb9ef54-31b7-4397-9eb8-ff8d3b587265";

    // Handles changes to form input fields.
    const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => setFormData({ ...formData, [e.target.name]: e.target.value });

    // Handles form submission to Web3Forms.
    const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            if (data.success) {
                ...
                formElement.reset(); // Resets the form fields.
                setFormData(initialFormData); // Resets the React state for form fields.
            } else {
            ...
            setTimeout(() => setResult(""), 6000); // Clear status message after a delay.
        }
    };
...
// --- Main App Component ---
/**
 * The main application component that manages routing and renders the active page.
 * It uses React's useState and useEffect hooks to handle client-side routing
 * and update the browser's history and document title.
 */
function App() {
    // State to keep track of the currently active page.
    const [activePage, setActivePage] = useState<PageName>("Home");
    // State to hold the selected project details when navigating to ProjectDetails page.
    const [selectedProject, setSelectedProject] = useState<Project | null>(null);

    /**
     * Navigates to a specified page and updates the browser history and document title.
     * @param page The name of the page to navigate to.
     * @param project Optional: The project object if navigating to a ProjectDetails page.
     */
    const navigateToPage = (page: PageName, project: Project | null = null) => {
        setActivePage(page);
        setSelectedProject(project);
        let url = '/'; // Default URL for home.
        let title = 'West Windsor Forward'; // Default title.

        // Determine the URL and title based on the active page.
        switch (page) {
            ...
            case 'ProjectDetails':
                ...
                break;
            default: url = '/'; title = 'West Windsor Forward'; // Fallback to home.
        }

        // Update browser history if the URL is different from the current one.
        // This prevents unnecessary history entries for the initial load.
        ...
        // Update document title for SEO and user experience.
    ...
    // useEffect hook for handling browser's popstate event (back/forward buttons)
    // and initial page load based on the URL.
    useEffect(() => {
        // Listener for browser history changes.
        const handlePopState = (event: PopStateEvent) => {
            if (event.state) {
                // If state exists, update active page and selected project from history state.
        ...
        // Logic for initial page load based on the URL path.
        const path = window.location.pathname;
        const parts = path.split('/').filter(Boolean); // Split path and remove empty strings.

        // Navigate to the appropriate page based on the URL segments.
        if (parts[0] === 'about') navigateToPage('About');
        else if (parts[0] === 'projects') {
            if (parts[1]) { // If there's a second part, it's a project slug.
                const project = projectsData.find(p => p.slug === parts[1]);
                if (project) navigateToPage('ProjectDetails', project);
                else navigateToPage('Projects'); // Fallback if project not found.
            } else navigateToPage('Projects'); // If only '/projects', show all projects.
        } else if (parts[0] === 'events') navigateToPage('Events');
        else if (parts[0] === 'contact') navigateToPage('Contact');
        else if (parts.length === 0) navigateToPage('Home'); // Root path.
    ...
    /**
     * Renders the appropriate page component based on the `activePage` state.
     */
    const renderPage = () => {
        // Special handling for ProjectDetails page as it requires a `selectedProject` prop.
        if (activePage === "ProjectDetails" && selectedProject) {
            return <ProjectDetailPage project={selectedProject} setActivePage={navigateToPage} />;
        }
        // Render other pages based on the activePage state.
        switch (activePage) {
            ...
            default: return <HomePage setActivePage={navigateToPage} />; // Fallback.
        }
    };
...
// Export the App component as default for React rendering.
export default App;

// --- Style injection ---
// This block injects custom CSS and Google Fonts into the document's head.
// This is done programmatically to ensure styles are applied when the React app runs in an isolated environment.
if (typeof window !== "undefined") {
